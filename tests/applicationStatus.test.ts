...
import { provenanceForSlug, type ProgramProvenance } from "@/data/sources";
...
describe("statusForWindow", () => {
  ...
  it("current open window → open", () => {
    expect(
      statusForWindow({ opens: "2026-06-01", closes: "2026-07-01" }, NOW),
    ).toBe("open");
  });

  it("future window → upcoming", () => {
    expect(
      statusForWindow({ opens: "2026-08-01", closes: "2026-09-01" }, NOW),
    ).toBe("upcoming");
  });

  it("past window → closed", () => {
    expect(
      statusForWindow({ opens: "2026-01-01", closes: "2026-03-01" }, NOW),
    ).toBe("closed");
  });
  ...
  it("boundary: exactly on open date → open", () => {
    expect(
      statusForWindow({ opens: "2026-06-11", closes: "2026-07-01" }, NOW),
    ).toBe("open");
  });

  it("boundary: exactly on close date → open (not yet past)", () => {
    expect(
      statusForWindow({ opens: "2026-06-01", closes: "2026-06-11" }, NOW),
    ).toBe("open");
  });
});
...
describe("resolveApplicationStatus precedence", () => {
  it("window overrides legacy status", () => {
    ...
    const r = resolveApplicationStatus(
      { legacyStatus: "rolling", windows },
      NOW,
    );
...
it("missing windows fall back to legacy status instead of erroring", () => {
  expect(
    resolveApplicationStatus({ legacyStatus: "open", windows: undefined }, NOW),
  ).toEqual({
    status: "open",
    source: "legacy",
  });
  expect(
    resolveApplicationStatus(
      { legacyStatus: undefined, windows: undefined },
      NOW,
    ),
  ).toEqual({
    status: "unknown",
...
describe("provenance data", () => {
  it("does not keep stale sample sources for removed startup-support programs", () => {
    for (const slug of [
      "y-combinator",
      ...
      expect(provenanceForSlug(slug)).toBeUndefined();
    }
  });
});
