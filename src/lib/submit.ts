...
export function buildIssueBody(f: SubmitFields): string {
  const lines: string[] = [];
  lines.push(f.mode === 'update' ? `**Update to an existing live-in founder program**` : `**New live-in founder program submission**`);
  lines.push('');
  ...
  lines.push('', '_Submitted via the 0rbital /submit form. Please verify this live-in founder program against primary sources before merging._');
  return lines.join('\n');
}

export function buildIssueUrl(f: SubmitFields): string {
  const title = (f.mode === 'update' ? '[Update] ' : '[New live-in founder program] ') + (f.name || 'Untitled');
