// Filtering + sorting — ported from the legacy passes()/sort logic so the map,
// globe and dashboard all agree on what a given filter set returns.

import type { Program } from '../data/programs';
import { programSectors } from '../data/sectors';
import { STATUS_ORDER, type StatusKey } from './status';

export interface Filters {
  /** Free-text search across program identity, location, focus, notes, tags and structured fit fields. */
  q: string;
  /** Recruiting status (open, coming-soon, running, closed) or '' for any. */
  status: string;
  /** Selected sector ids (see data/sectors.ts), OR-matched. Empty = any. */
  sector: string[];
  /** Selected countries, OR-matched. Empty array = no country constraint. */
  country: string[];
  /** Living model / format values (`live-in`, `relocation`, `hybrid`, `unknown`, ...). */
  format: string[];
  /** Require housing to be explicitly provided. */
  housing: boolean;
  /** Require workspace to be explicitly provided. */
  workspace: boolean;
  /** Require funding to be explicitly provided. */
  funding: boolean;
}

export const EMPTY_FILTERS: Filters = {
  q: '',
  status: '',
  sector: [],
  country: [],
  format: [],
  housing: false,
  workspace: false,
  funding: false,
};

const text = (v: unknown): string => {
  if (Array.isArray(v)) return v.join(' ');
  if (typeof v === 'string') return v;
  if (typeof v === 'number' || typeof v === 'boolean') return String(v);
  return '';
};

function searchHaystack(p: Program): string {
  return [
    p.name,
    p.city,
    p.country,
    p.focus,
    p.operator,
    p.type,
    p.canonicalType,
    p.format,
    p.highlight,
    p.status_detail,
    p.notes,
    p.stage,
    p.stageFit,
    p.founderFit,
    p.sectorFocus,
    p.supportModes,
    p.tags,
  ]
    .map(text)
    .join(' ')
    .toLowerCase();
}

/** Sector + country + status + living model + explicit perks + free-text search. */
export function passes(p: Program, f: Filters): boolean {
  const q = f.q.trim().toLowerCase();
  const hay = searchHaystack(p);
  return (
    (!q || hay.includes(q)) &&
    (!f.sector.length || programSectors(p).some((s) => f.sector.includes(s))) &&
    (!f.country.length || f.country.includes(p.country)) &&
    (!f.status || p.status === f.status) &&
    (!f.format.length || f.format.includes(p.format ?? 'unknown')) &&
    (!f.housing || p.providesHousing === true || !!p.supportModes?.includes('housing')) &&
    (!f.workspace || p.providesWorkspace === true || !!p.supportModes?.includes('workspace')) &&
    (!f.funding || p.providesFunding === true || !!p.supportModes?.includes('funding'))
  );
}

export function hasActiveFilters(f: Filters): boolean {
  return Boolean(
    f.q ||
      f.status ||
      f.sector.length ||
      f.country.length ||
      f.format.length ||
      f.housing ||
      f.workspace ||
      f.funding,
  );
}

export type SortKey = keyof Program;

export function sortPrograms(list: Program[], sort: SortKey, dir: 1 | -1): Program[] {
  return [...list].sort((a, b) => {
    let av: number | string;
    let bv: number | string;
    if (sort === 'status') {
      av = STATUS_ORDER.indexOf(a.status as StatusKey);
      bv = STATUS_ORDER.indexOf(b.status as StatusKey);
    } else {
      av = String(a[sort] ?? '').toLowerCase();
      bv = String(b[sort] ?? '').toLowerCase();
    }
    if (av < bv) return -1 * dir;
    if (av > bv) return 1 * dir;
    return a.name.localeCompare(b.name);
  });
}

/** Default ordering used by the legacy list: status order, then name. */
export function defaultSort(list: Program[]): Program[] {
  return sortPrograms(list, 'status', 1);
}
